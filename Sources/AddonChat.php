...
class AddonChat
{
	...
	public static function menu(&$menu_buttons)
	{
		...
		$menu_buttons = array_merge(
			...
			array('chat' => array(
			'title' => $tools->getText('title_main') . ($tools->enable('show_chatusers_menu') && !empty($whos) ? ' ['. (!is_array($whos) ? '0' : $whos['number']) .']' : ''),
	...
	static function lookSettings($return_config = false)
	{
		...
		$config_vars = array(
			...
			array('check', self::$name .'_boardIndex_rooms', 'subtext' => $tools->getText('show_chatusers_boardIndex_rooms_sub')),
		...
		if (isset($_GET['save']))
		{
			checkSession();
			saveDBSettings($config_vars);
			redirectexit('action=admin;area=AddonChat;sa=look');
		}
